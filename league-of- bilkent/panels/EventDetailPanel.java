...
// shows everything about a single event when you click on it from the feed
// has event info, rsvp buttons, attendee list, and threaded comments
// event = which event we're showing, homeScreen = for navigation
public class EventDetailPanel extends JPanel {
    ...
    // builds the whole page top to bottom, wrapped in a scroll pane
    private JScrollPane createBody() {
        ...
        // event properties, some only show if they have data
        ...
        // delete button, only the creator can see this
    ...
    // label = left side text like "Location", value = right side like "SA-Z18"
    ...
    // rsvp buttons that toggle on/off
    // going checks deadline + tier, interested and maybe dont
    private JPanel buildAttendanceSection() {
        ...
        btnGo.addActionListener(e -> {
            if (current == AttendanceStatus.GOING) {
                ...
                if (deadlinePassed) {
                    UIHelper.showError(this, AppConstants.ERR_DEADLINE_PASSED);
                    return;
                }
                if (!event.canJoin(Database.getUserXP(me))) {
                    UIHelper.showError(this, "You need " + event.getMinTierName() + " tier!");
                    return;
                }
    ...
    // selected = filled button with checkmark, not selected = outline
    ...
    // shows whos attending with clickable usernames that go to their profile
    ...
    // threaded comments, top-level first then replies indented under them
    // only top-level comments can be replied to (1 level deep)
    private JPanel buildCommentsSection() {
        ...
        ArrayList<Comment> topLevel = new ArrayList<>();
    ...
    // comment = the comment to display, indent = 0 for top-level, 1 for reply
    // replies get lighter background + extra left padding
        ...
        if (commenter != null) {
            ...
            nameLbl.addMouseListener(new java.awt.event.MouseAdapter() {
                public void mouseClicked(java.awt.event.MouseEvent e) {
                    homeScreen.navigateToProfile(commenter);
                }
            });
            topRow.add(nameLbl);
        }
    ...
    // parentId = 0 for new top-level comment, > 0 for reply to that comment
    // works with both send button click and pressing enter
