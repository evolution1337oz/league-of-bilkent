...
public class CreateEventPanel extends JPanel {
    ...
    private void handleCreate() {
        ...
        if (t.isEmpty()) {
            UIHelper.showError(this, AppConstants.ERR_TITLE_REQUIRED);
            return;
        }

        String loc = UIHelper.getFieldText(locationField, AppConstants.PH_LOCATION);
        if (loc.isEmpty()) {
            UIHelper.showError(this, AppConstants.ERR_LOC_REQUIRED);
            return;
        }

        try {
            LocalDateTime start = LocalDateTime.of(
                (int) yearS.getValue(), (int) monthS.getValue(),
                (int) dayS.getValue(), (int) hourS.getValue(), (int) minS.getValue());

            LocalDateTime end = LocalDateTime.of(
                (int) yearE.getValue(), (int) monthE.getValue(),
                (int) dayE.getValue(), (int) hourE.getValue(), (int) minE.getValue());

            LocalDateTime deadline = LocalDateTime.of(
                (int) deadlineYear.getValue(), (int) deadlineMonth.getValue(),
