...
public class SearchPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        header.setLayout(new BoxLayout(header, BoxLayout.Y_AXIS));//vertical stacking
        ...
        for (String cat : AppConstants.INTEREST_CATEGORIES) {
            ...
            chip.addActionListener(e -> { searchField.setText(cat.toLowerCase()); performSearch(); });//searching happens
            tagRow.add(chip);
        }
    ...
    private void performSearch() {
        resultsPanel.removeAll();//initilization and clearing
        ...
        for (Event ev : events) {
            if (ev.matchesSearch(query)) {
                ...
                btn.addMouseListener(new java.awt.event.MouseAdapter() {//hover effect implementation
        ...
        for (User u : users) {
            if (u.matchesSearch(query)) {//checking whether match found
                foundUser = true;
